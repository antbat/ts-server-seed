...
import { RootController } from './domains/healthcheck/root.controller';
...
export class App {
    ...
    public listen(port: number) {
        this.application.listen(port, () => {
            logger.info(`the seed application are listening on ${port}`);
        });
    }
}
...
export async function getApp(): Promise<App> {
    ...
    const controllers = [new RootController()];
